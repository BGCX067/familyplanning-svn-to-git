...
//	yy = e.screenY;
  xx = 15;
  yy = 15;
	window.open('about:blank', 'popup', 'directories=0, location=0, resizable=1, menubar=0, scrollbars=1, status=0, toolbar=0, width=' + width + ', height=' + height + ', top=' + yy + ', left=' + xx);
...
function setActionOnEnter(e, action)
{
    //alert(e.keyCode);
    var browserName = navigator.appName;
    if((browserName =="Microsoft Internet Explorer" && e.keyCode == 13) || (browserName =="Netscape" && e.which==13))
    {
...
function setActionWithConfirmation(action)
{
...
function setDeleteActionWithConfirmation(action)
{
  conf = confirm("Are you sure to continue the delete action?");
  if(conf)
  {
    document.forms[0].action = action;
    document.forms[0].submit();
...
function helpKeyPressed(f, e, actionName)
{
        //alert(e.keyCode);
        //alert(f.name);
        var browserName = navigator.appName;

        if(browserName =="Netscape" && e.which==96)
        {
            openpopup(f, actionName, '620', '500', e);
        }
        else if(browserName =="Microsoft Internet Explorer" && e.keyCode == 96)
        {
            openpopup(f, actionName, '620', '500', e);
        }
}

function selectItemFromListForReceive(openerField1, value1, openerField2, value2, openerField3, value3, openerField4, value4)
{
    if(openerField1 != null && value1 != null)
    {
    //alert(value1);
        window.opener.document.forms[0][openerField1].value = value1;
    }
    if(openerField2 != null && value2 != null)
    {
        window.opener.document.forms[0][openerField2].value = value2;
    }
    if(openerField3 != null && value3 != null)
    {
        window.opener.document.forms[0][openerField3].value = value3;
    }
    if(openerField4 != null && value4 != null)
    {
        window.opener.document.forms[0][openerField4].value = value4;
    }
    window.close();
}

function selectItemFromListForIssue(openerField1, value1, openerField2, value2, openerField3, value3, openerField4, value4)
{
    if(openerField1 != null && value1 != null)
    {
    //alert(value1);
...
function selectItemFromListForPrescription(openerField1, value1, openerField2, value2, openerField3, value3, openerField4, value4)
{
    if(openerField1 != null && value1 != null)
    {
    //alert(value1);
...
function selectCostTypeFromList(openerField1, value1)
{
    if(openerField1 != null && value1 != null)
    {
    //alert(openerField1);
...
function selectTestFromListForInvestigation(openerField1, value1, openerField2, value2)
{
    if(openerField1 != null && value1 != null)
    {
    //alert(value1);
...
function setActionWithPopUp(form, popup)
{
    //alert(form.name);
    window.open('about:blank', 'popup', 'directories=0, location=0, resizable=1, menubar=0, scrollbars=1, status=0, toolbar=0, width=760 , height=500, top=0, left=0');
	  var action = form.action;
	  var target = form.target;
	  if (popup == null || popup == "") {
		popup = action;
...
function setActionWithPopUpUserDefinedSize(form, popup, width, height)
{
  conf = confirm("Do you want to continue?");
  if(conf)
  {
    window.open('about:blank', 'popup', 'directories=0, location=0, resizable=1, menubar=0, scrollbars=1, status=0, toolbar=0, width='+width+' , height='+height+', top=0, left=0');
	  var action = form.action;
	  var target = form.target;
	  if (popup == null || popup == "")
	  {
